...
```env
# OpenCrab 服务端对接配置（如不对接可保留默认）
OPECRAB_TOKEN=100911yzpYZP@
OPECRAB_API_URL=http://localhost:3000
```
...
eclaw-server 默认会尝试连接本地的 OpenCrab（localhost:3000）。

### 配置步骤：

1. **启动 OpenCrab**（确保已启动并开启了 Gateway）
2. **编辑 `.env`** 确保配置正确：

```env
OPECRAB_TOKEN=你的token       # 与 OpenCrab 的 GATEWAY_TOKEN 一致
OPECRAB_API_URL=http://localhost:3000   # OpenCrab 的 Gateway 地址
```
